...
<body>
	...
	<div class="profile">
		...
		<c:if test="${sessionScope.member_id != null }">
			<a href="<%=request.getContextPath()%>/product/write.do"
				style="margin-left: 200px; margin-top: 150px;"><button
					type="button" class="btn btn-sm bg-dark text-white">상품등록</button></a>
		</c:if>
									...
									<div>
										...
										<div class="product-pic" style="margin-top: 50px;">
											<a
												...
												<img class="pic1" src="../resources/images/${row.filename}"
										...
										<div style="margin-top: 100px; text-align: center;">
											<small class="category" style="margin-top: 100px;">${row.product_brand}</small><span>
												<c:if test="${sessionScope.member_id != null }">
													<a
														href="<%=request.getContextPath()%>/product/edit/${row.product_code}"><button
															type="button" class="btn btn-sm bg-dark text-white">수정</button></a>
											</span>
											</c:if>
											<a
												href="<%=request.getContextPath()%>/product/detail/${row.product_code}"
											...
											<div class="row px-3 justify-content-between">
												<p class="price">
													<fmt:formatNumber value="${row.product_price}"
														pattern="#,###" />
													&nbsp;원
												</p>
	...
	<ul class="pagination justify-content-center" id="pageInfo">
		...
		<c:if test="${pageMaker.prev}">
			<li class="page-item disabled"><a class="page-link"
				href="<%=request.getContextPath() %>/product/list.do?page=${pageMaker.startPage - 1}&brand=${pageMaker.cri.brand}">Previous</a>
			</li>
		</c:if>
		...
		<c:forEach var="num" begin="${pageMaker.startpage}"
			...
			<li class="page-item"><a class="page-link"
				href="<%=request.getContextPath() %>/product/list.do?page=${num}&brand=${pageMaker.cri.brand}&keyword=${pageMaker.cri.keyword}">${num}</a>
			</li>
		</c:forEach>

		<c:if test="${pageMaker.next}">
			<li class="page-item"><a class="page-link"
				href="<%=request.getContextPath() %>/product/list.do?page=${pageMaker.endPage + 1}&brand=${pageMaker.cri.brand}">Next</a>
			</li>
		</c:if>
	</ul>
